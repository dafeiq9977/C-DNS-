...
#include <sys/time.h>
#define PORT 53
const char *nextserver = "127.0.0.3"; //默认指向root 
...
unsigned int questResult(char *rec, int recLen, char *res){
	...
	dns_query *query = (dns_query *)malloc(sizeof(dns_query));printf("come here\n");// initQuery(query);
	char *q=rec; char *r=res;
	q += 2+sizeof(dns_header);
	q += downQuery(q, query);
	printf("query name=%s\n", query->name);
	r+=2+sizeof(dns_header); //跳过TCP长度的两个字节和头部字段 
	r+=upQuery(r, query);
	resLen += r-res;
...
void addToCache(char *res){
	...
	while(rrNum>=1){
		...
		printf("before, rRecord->name is %s\n",rRecord->name);
		fprintf(fp, "\n");
		nameSwitch(rRecord->name);
		printf("rRecord->name is %s\n",rRecord->name);
...
unsigned int searchCache(char *name, char *r, dns_header *head){
	...
	while(fgets(col, DNSMAXLEN-1, cache) != NULL){
		if(cmpDomainName(name, col, now.tv_sec)){
			...
			p+=blocklen(p); p+=blocklen(p); //跳过name和有效时间字段
...
unsigned int downRR(char *buf, char *cur, dns_rr *rRecord){
	...
	if(cur[0]==pointer){
		char *tmp=cur;
		cur = buf+cur[1];
		len = strlen(cur)+1;printf("name len=%d\n",len);
		rRecord->name = (char *)malloc(len*sizeof(char));
		memcpy(rRecord->name, cur, len);printf("name=%s\n",rRecord->name);
		cur = tmp + 2; offset+=2;
	}else{
		len =strlen(cur)+1;printf("name len=%d\n",len);
		rRecord->name = (char *)malloc(len*sizeof(char));
